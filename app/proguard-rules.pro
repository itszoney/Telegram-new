...
# ── License-key & anti-tampering security layer ───────────────────────────────

# Keep security package (LicenseKeyManager, IntegrityChecker, NativeSecurityBridge)
-keep class org.thunderdog.challegram.security.** { *; }
-keepclassmembers class org.thunderdog.challegram.security.** { *; }

# Keep LicenseGateController (used via explicit Intent, not reflection)
-keep class org.thunderdog.challegram.ui.LicenseGateController { *; }

# Native method in NativeSecurityBridge (must match JNI symbol exactly)
-keepclasseswithmembers class org.thunderdog.challegram.security.NativeSecurityBridge {
    private native boolean nativeVerifyCert(byte[]);
}
...
# Play Integrity (already a dep; keep result classes)
-keep class com.google.android.play.core.integrity.** { *; }
-dontwarn com.google.android.play.core.integrity.**

# R8 full mode: keep Kotlin metadata so serialization works
-keepattributes *Annotation*, Signature, InnerClasses, EnclosingMethod

# ── End security layer rules ──────────────────────────────────────────────────