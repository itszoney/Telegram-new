...
public class AudioTrackJNI{
	...
	private final AudioEffectsProcessor audioEffects;

	public AudioTrackJNI(long nativeInst) {
		this.nativeInst = nativeInst;
		this.audioEffects = new AudioEffectsProcessor(VoIPAudioEffects.INSTANCE.getConfig());
	}
				...
				while (running) {
					try {
						if(needResampling){
							...
							audioEffects.process(buffer, 0, buffer.length);
							tmp48.rewind();
							tmp48.put(buffer);
							...
							nativeCallback(buffer);
							audioEffects.process(buffer, 0, 960*2);
